...
func TestTCP(t *testing.T) {
	...
	assert.Nil(t, n1.Dial(n2.Addr()))

	assert.Nil(t, n1.Send(n2.Addr(), []byte("hello")))
	time.Sleep(time.Second * 5)
}

func MonitorNode(prefix string, peerCh chan p2p.Peer, errCh chan error, msgCh chan string, rpcCh chan *p2p.RPC) {
	for {
		select {
		case peer := <-peerCh:
...
func MakeTCPTRansport(
	...
	node, err := p2p.NewTCPTransport(addr, peerCh, errCh, msgCh, rpcCh)
	assert.Nil(t, err)
	go node.Start()
